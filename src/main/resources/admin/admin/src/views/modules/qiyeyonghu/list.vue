        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '企业用户名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.qiyeyonghuName" placeholder="企业用户名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('qiyeyonghu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/zhihuigongyuguanli/upload/qiyeyonghuMuBan.xls"
                        >批量导入企业用户数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('qiyeyonghu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="qiyeyonghuUploadSuccess"
                                :on-error="qiyeyonghuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('qiyeyonghu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入企业用户数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('qiyeyonghu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "qiyeyonghu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="企业用户名称">
                        <template slot-scope="scope">
                            {{scope.row.qiyeyonghuName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="qiyeyonghuPhone"
                                   header-align="center"
                                   label="企业用户手机号">
                        <template slot-scope="scope">
                            {{scope.row.qiyeyonghuPhone}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="qiyeyonghuPhoto"
                                     header-align="center"
                                     width="200"
                                     label="企业用户头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.qiyeyonghuPhoto">
                                <img :src="$base.url+scope.row.qiyeyonghuPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="qiyeyonghuEmail"
                                   header-align="center"
                                   label="企业用户邮箱">
                        <template slot-scope="scope">
                            {{scope.row.qiyeyonghuEmail}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('qiyeyonghu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('qiyeyonghu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('qiyeyonghu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('qiyeyonghu','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                password : null,
                qiyeyonghuName : null,
                qiyeyonghuPhone : null,
                qiyeyonghuPhoto : null,
                sexTypes : null,
                qiyeyonghuEmail : null,
                dataDelete : null,
                insertTime : null,
                createTime : null,
            },
            ...
            json_fields: {
                ...
                //本表字段
                     '账户': "username",
                     '企业用户名称': "qiyeyonghuName",
                     '企业用户手机号': "qiyeyonghuPhone",
                     '企业用户头像': "qiyeyonghuPhoto",
                     '性别': "sexValue",
                     '企业用户邮箱': "qiyeyonghuEmail",
                     '添加时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'qiyeyonghu',//当前表表名,
                        sumColum : 'qiyeyonghu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'qiyeyonghu_name',//分组字符串字段
                        // types : 'qiyeyonghu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.qiyeyonghuName!= '' && this.searchForm.qiyeyonghuName!= undefined) {
                    params['qiyeyonghuName'] = '%' + this.searchForm.qiyeyonghuName + '%'
                }
                ...
                params['qiyeyonghuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "qiyeyonghu/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            qiyeyonghuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "qiyeyonghu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "qiyeyonghu/update",
                        method: "post",
                        data: {
                            id:id,
//                            qiyeyonghuTypes:qiyeyonghuTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
